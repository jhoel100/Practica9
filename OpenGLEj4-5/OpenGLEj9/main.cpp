...
void init(GLFWwindow* window) {
	...
	cameraX = 0.0f; cameraY = 15.0f; cameraZ = 60.0f;
	setupVertices();
}
...
void display(GLFWwindow* window, double currentTime) {
	...
	mvStack.push(mvStack.top());  // mvMat of sun
	mvStack.top() *= glm::translate(glm::mat4(1.0f), glm::vec3(sin((float)currentTime*0.5) * 20.0,
		0.0f,
		cos((float)currentTime*0.5) * 20.0));  // planet position
	mvStack.push(mvStack.top());  // duplicating
	mvStack.top() *= glm::rotate(glm::mat4(1.0f), (float)currentTime, glm::vec3(0.0, 1.0, 0.0));  // planet rotation
